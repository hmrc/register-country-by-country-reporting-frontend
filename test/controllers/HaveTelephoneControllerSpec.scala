...
class HaveTelephoneControllerSpec extends SpecBase {
  ...
  "HaveTelephone Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = baseUserAnswers.set(HaveTelephonePage, true).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(true), firstContactName, NormalMode)(request, messages(application)).toString
      }
    }
